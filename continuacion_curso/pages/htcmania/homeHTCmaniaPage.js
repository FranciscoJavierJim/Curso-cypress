class homeSaucePage{        // declara una clase llamada 'homeSaucePage' que usaremos con page object para la pantalla de login
    elements = {            // crea una propiedad de clase llamada 'elements'. Aqui agruparemos funciones que devuelven localizadores de cypress
        usernameInput: () => cy.get('.alt1>.smallfont>[name="vb_login_username"]'), // es una funcion que, cuando la llamamos, ejecuta 'cy.get...' para obtener el campo de usuario por su id
        passwordInput: () => cy.get('.alt1>.smallfont>[name="vb_login_password"]'),  // igual que arriba, pero para el campo de la contraseña
        loginButton: () => cy.get('div[style*="padding-top:6px"] input.button'),// devuelve el boton de login
